...
# Фаза 5 — без человека в цепочке: демон постит заказы, нанимает, судит, рассчитывает
# и каждый тик копит репутацию, затем печатает лидерборд воркеров.
...
- **Daemon + репутация** (Фаза 5) — `agent-x daemon` крутит цикл без человека: постит
  заказы и рассчитывает в X402 каждый тик, а стойкая **репутация** (средний балл +
  winrate + доказанный объём → 0–100) решает близкие бейк-оффы. Воркеры без капитала
  перестают делать ставки и уходят с рынка.

Состояние лежит в `.agentx/` (переопределяется через `AGENT_X_DATA_DIR`). Архитектуру
и полную карту фаз см. в [`ROADMAP.ru.md`](./ROADMAP.ru.md).
